(function($) {
	$.fn.extend({
		passValidate: function(_options, _callback, _confirmcallback) {
			...
			function buildPasswordRegex() {
				...
				if((_options.noUpper || _options.noLower || _options.noDigit)) {
					if(_options.failConsecutive) {
						cases.push({"regex": "(?!.*([" + charsetToString() + "])\1{" + _options.maxConsecutive + "})",	"message": "Password can't contain the same character more than " + _options.maxRepeats + " times in a row."});
					}
				}
			...
			function validateField() {
				...
				_callback(_element, validPassword, fieldsMatch, failedCases);
			}
